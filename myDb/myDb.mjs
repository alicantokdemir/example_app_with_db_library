 ...
 */
export function addTable(db, tableName) {
  if (!db) {
 ...
 * @returns {Object} Updated database object
 */
export function addEntity(db, tableName, entityId, entityBody) {
  if (!db) {
    throw 'Invalid database';
  }
 ...
 */
export function resolveReference(db, [tableName, entityId, fieldName] = []) {
  if (!db) {
    throw 'Invalid database';
  }
 ...
 * @returns {Object} Database object
 */
export function denormalize(db) {
  if (!db) {
    throw 'Invalid database';
  }
